## Flask 文件管理系统教程（支持多文件上传）## 用途说明本项目是一个基于 Flask 框架开发的文件管理小工具，适用于个人、小型团队或家庭局域网环境。你可以方便地通过网页上传、下载、删除文件，并浏览所有已上传的文件。**支持多文件上传**，适合日常文件归档、资料共享等场景。---## 1. 项目结构
```
file_manager/
│
├── app.py
├── uploads/               # 存储上传文件的目录（需提前创建）
└── templates/
    └── file_manager.html  # 主页面HTML模板
```
> 提示：请自行创建 `uploads/` 文件夹，并确保有写权限。
...
<body>
    ...
    <form id="upload-form" action="/" method="post" enctype="multipart/form-data">
        <label for="file-input">选择文件（可多选）：</label>
    ...
    <div class="file-list">
        ...
        <ul>
            ...
            <li class="file-item">
                {{ file }}
                <a class="action-link" href=" 'download_file', filename=file) }}">下载</a >
                <a class="action-link" href="{{ url_for('delete_file', filename=file) }}" onclick="return confirm('确认删除 {{ file }} 吗？');">删除</a >
            </li>
            {% else %}
            <li>暂无文件</li>
            {% endfor %}
...
确保在 `file_manager/` 目录下，终端运行：

```bash
python app.py
```

然后在浏览器输入 [http://127.0.0.1:5000/](http://127.0.0.1:5000/) 打开页面。
...
- **上传**  
  点击“选择文件”，可通过多选上传任意数量的文件，随后点击“上传”按钮即可完成上传。

- **下载**  
  在文件列表点击“下载”，即可下载对应文件到本地。

- **删除**  
  点击“删除”即可删除对应文件，操作有二次确认。

- **浏览列表**  
  所有已上传文件在页面“已上传文件”栏中列出，便于查找和管理。
...
function genListHtml(files) {
  ...
  for(let f of files){
    ...
    if(f.type==="folder") {
      ...
      li.innerHTML = `<span class='me-auto' onclick='loadPath("${makePath(currentPath, f.name)}")'><b>📁 ${f.name}</b></span>
        <button class='icon-btn ms-2' onclick='delEntry("${f.name}","folder")'><span class="text-danger">🗑️</span></button>`;
      // Drag events
    }else{
      li.innerHTML = `<span class='me-auto'>📄 ${f.name}</span>
        <a href="/api/download?path=${encodeURIComponent(makePath(currentPath, f.name))}">
           <button class='icon-btn'>⬇️</button>
        </a >
        <button class='icon-btn ms-2' onclick='delEntry("${f.name}","file")'><span class="text-danger">🗑️</span></button>`;
    }
    list.appendChild(li);
  }
}
